...
def create_cost_and_derivates(manifold, ABt, backend):
    ...
    if backend == "autograd":
        ...
        def cost(X):
            return -anp.tensordot(X, ABt, axes=X.ndim)
...
def run(backend=SUPPORTED_BACKENDS[0], quiet=True):
    ...
    Xinit = np.random.normal(size=(k, n, n))
    # only numpy, jax, pytorch for now
    if backend == 'jax':
        Xinit = jnp.asarray(Xinit)
    elif backend == 'pytorch':
        Xinit = torch.from_numpy(Xinit)


    optimizer = SteepestDescent(max_iterations=2, verbosity=2 * int(not quiet))
    X = optimizer.run(problem, initial_point=Xinit).point

    if not quiet:
        Xopt = np.array([compute_optimal_solution(ABtk) for ABtk in ABt])
        print("Frobenius norm error:", np.linalg.norm(Xopt - X))

    print(f'\nX input  shape/type: {Xinit.shape} / {type(Xinit)}')
    print(f'X output shape/type: {X.shape} / {type(X)}')
