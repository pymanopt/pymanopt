...
class TestMulti(TestCase):
    ...
    def test_multiprod_singlemat(self):
        ...
        B = np.random.randn(self.n, self.p)
    ...
    def test_multiprod(self):
        ...
        A = np.random.randn(self.k, self.m, self.n)
        B = np.random.randn(self.k, self.n, self.p)
    ...
    def test_multisym(self):
        A = np.random.randn(self.k, self.m, self.m)
    ...
    def test_multiexp_singlemat(self):
        # A is a positive definite matrix
        A = np.random.randn(self.m, self.m)
        A = A + A.T
        np_testing.assert_allclose(multiexp(A, sym=True), expm(A))

    def test_multiexp(self):
        A = multisym(np.random.randn(self.k, self.m, self.m))
