...
class TestFixedRankEmbeddedManifold(TestCase):
    ...
    def test_inner(self):
        ...
        A = x[0] @ a[1] @ x[2].T + a[0] @ x[2].T + x[0] @ a[2].T
        B = x[0] @ b[1] @ x[2].T + b[0] @ x[2].T + x[0] @ b[2].T
    ...
    def test_proj_range(self):
        ...
        np_testing.assert_allclose(g[0].T @ x[0],
                                   ...
                                   atol=1e-6)
        np_testing.assert_allclose(g[2].T @ x[2],
                                   np.zeros((self.k, self.k)),
                                   atol=1e-6)
    ...
    def test_rand(self):
        ...
        np_testing.assert_allclose(x[2].T @ x[2], np.eye(self.k), atol=1e-6)
    ...
    def test_tangent2ambient(self):
        ...
        z_ambient = x[0] @ z[1] @ x[2].T + z[0] @ x[2].T + x[0] @ z[2].T
    ...
    def test_retr(self):
        ...
        np_testing.assert_allclose(y[2].T @ y[2], np.eye(self.k), atol=1e-6)
    ...
    def test_egrad2rgrad(self):
        print("TODO: This needs new tests")
    ...
    def test_randvec(self):
        ...
        np_testing.assert_allclose(u[0].T @ x[0],
                                   np.zeros((self.k, self.k)),
                                   atol=1e-6)
        np_testing.assert_allclose(u[2].T @ x[2],
                                   np.zeros((self.k, self.k)),
                                   atol=1e-6)
